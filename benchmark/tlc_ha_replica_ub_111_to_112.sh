...
reset_command="TLC_HA_UB_TX_PATH='$NODE111_TX_PATH' TLC_HA_UB_RX_PATH='$NODE111_RX_PATH' TLC_HA_UB_TX_OFFSET='$TX_OFFSET' TLC_HA_UB_RX_OFFSET='$RX_OFFSET' TLC_HA_UB_RESET=1 TLC_HA_UB_RESET_ONLY=1 timeout '${TEST_TIMEOUT_SECONDS}s' ./benchmark/tlc_ha_replica_ub_node_ut follower"
reset_peer_command="TLC_HA_UB_TX_PATH='$NODE112_TX_PATH' TLC_HA_UB_RX_PATH='$NODE112_RX_PATH' TLC_HA_UB_TX_OFFSET='$RX_OFFSET' TLC_HA_UB_RX_OFFSET='$TX_OFFSET' TLC_HA_UB_RESET=1 TLC_HA_UB_RESET_ONLY=1 timeout '${TEST_TIMEOUT_SECONDS}s' ./benchmark/tlc_ha_replica_ub_node_ut follower"

reset_rings() {
    remote "$NODE111_PORT" "$NODE111_SSH" "$reset_command"
    remote "$NODE112_PORT" "$NODE112_SSH" "$reset_peer_command"
    # Allow the device-backed reset stores to become visible before attaching
    # a producer and consumer to the fresh ring headers.
...
if [ "${TLC_HA_UB_SKIP_SYNC:-0}" = 1 ]; then
    printf 'using existing remote UB binaries (TLC_HA_UB_SKIP_SYNC=1)\n'
else
...
grep -q 'HA resync handoff complete:.*H=97' "$LEADER_LOG.m6-retention"
