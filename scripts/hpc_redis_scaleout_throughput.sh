...
# 固定宽度汇总表头，所有列左对齐
summary_row phase ops_sec p50_ms p99_ms wall_s note > "$TSV"
...
verify_remote_prereqs() {
    local host=$1
    ssh_run "$host" "test -d $REMOTE_DIR && test -x $REMOTE_DIR/src/redis-server && test -x $REMOTE_DIR/benchmark/vemb_v16_topology_ctl && test -x $MEMTIER && ls $PAYLOAD_LOCAL_PATH $PAYLOAD_PEER_PATH $REQUEST_LOCAL_PATH $REQUEST_PEER_PATH $RESPONSE_LOCAL_PATH $RESPONSE_PEER_PATH >/dev/null"
}
...
record_phase() {
    ...
    summary_row "$phase" "$ops" "$p50" "$p99" "$wall" "$note" >> "$TSV"
...
read ops p50 p99 <<< "$result"
record_phase "during_scaleout" "$ops" "${p50:-NA}" "${p99:-NA}" "$SCALEOUT_WALL" "active={0}->{0,1}, memtier-client-topology-retry, endpoints=$FINAL_ENDPOINTS"

ssh_run "$NODE0_HOST" "cat $COORD_OUT; echo '---'; cat $COORD_ERR 2>/dev/null" || true

# 段3a: after scaleout, topology-routed read of the original migrated keyspace
log "段3a: after scaleout topology-routed old-key VEMB read (${TEST_TIME}s)"
T0=$(date +%s)
result=$(run_memtier "$NODE0_HOST" "$TEST_TIME" "$RAW_DIR/scaleout_after_old_keys.txt" \
    "" "$FINAL_ENDPOINTS" 1 "$PREFILL_KEYS")
T1=$(date +%s)
read ops p50 p99 <<< "$result"
record_phase "scaleout_after_old_keys" "$ops" "$p50" "$p99" "$((T1-T0))" "active={0,1}, automatic-topology, old_keys=1-$PREFILL_KEYS, endpoints=$FINAL_ENDPOINTS"

# 段3b: after scaleout, write/read new steady keyspace (active={0,1})
log "段3b: after scaleout new-key VEMB read (${TEST_TIME}s)"
T0=$(date +%s)
...
T1=$(date +%s)
read ops p50 p99 <<< "$result"
record_phase "scaleout_after" "$ops" "$p50" "$p99" "$((T1-T0))" "active={0,1}, automatic-topology, steady_keys=$STEADY_KEY_MIN-$STEADY_KEY_MAX, endpoints=$FINAL_ENDPOINTS"
