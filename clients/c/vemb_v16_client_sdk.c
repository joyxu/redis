...
static inline sdk_owner_stats_t *sdk_owner_stats(
...
static inline void sdk_owner_stats_note_pending_peak(
    vemb_v16_client_t *client, uint32_t owner_id, uint64_t pending)
{
    sdk_owner_stats_t *stats = sdk_owner_stats(client, owner_id);
    ...
    vemb_v16_client_handle_completion_cb cb, void *priv)
{
    ...
    request->route_next = session->route_head;
    session->route_head = request_id;
                                               ...
                                               uint64_t request_id)
{
    ...
    request->force_v1 = 1;
    request->route_next = session->route_head;
    session->route_head = (uint32_t)request_id;
}
    ...
    vemb_v16_client_handle_completion_cb cb, void *priv)
{
    ...
    uint32_t cur = session->route_head;
    session->route_head = SDK_HANDLE_SESSION_NO_ENTRY;
    while (cur != SDK_HANDLE_SESSION_NO_ENTRY) {
        uint32_t next = session->requests[cur].route_next;
        session->requests[cur].route_next = SDK_HANDLE_SESSION_NO_ENTRY;
        callbacks += sdk_handle_session_finish(session, cur, &error, cb,
                                               priv);
        cur = next;
    }
    return callbacks;
    ...
    vemb_v16_client_handle_completion_cb cb, void *priv)
{
    ...
    while (cur != SDK_HANDLE_SESSION_NO_ENTRY) {
        ...
        uint32_t next = request->route_next;
        request->route_next = SDK_HANDLE_SESSION_NO_ENTRY;
        ...
        if (prepared == VEMB_V16_CLUSTER_PREPARE_NEEDS_TOPOLOGY) {
            session->topology_refresh_pending = 1;
            request->route_next = next;
            session->route_head = cur;
            return callbacks;
        }
        ...
        sdk_owner_stats_note_pending_peak(session->client, route.owner_id,
                                          session->active_requests);
    ...
    uint32_t inline_bytes)
{
    ...
    sdk_owner_stats(session->client, owner_id)->submitted++;
    if (sdk_backend_read_warm_vector(
            ...
            expected_bytes) != 0) {
        sdk_owner_stats(session->client, owner_id)->errors++;
        *response = sdk_vector_session_error_response();
    } else {
        sdk_owner_stats(session->client, owner_id)->ok++;
    }
}
    ...
    vemb_v16_client_owner_stats_snapshot_t *out)
{
    ...
    _Static_assert(sizeof(sdk_owner_stats_t) ==
                   offsetof(vemb_v16_client_owner_stats_t, pending_current),
                   "internal/public stats layout mismatch");
    for (uint32_t owner = 0;
         owner < VEMB_V16_TOPOLOGY_CONTROL_MAX_ENDPOINTS; owner++) {
        memcpy(&out->owners[owner], &client->owner_stats[owner],
               sizeof(sdk_owner_stats_t));
        out->owners[owner].pending_current = 0;
        out->owners[owner].active_groups_current = 0;
    }
    if (client->handle_session) {
        for (uint32_t i = 0; i < SDK_HANDLE_SESSION_MAX_PENDING; i++) {
            const sdk_handle_session_request_t *r =
                &client->handle_session->requests[i];
            if (r->state != 0) {
                out->owners[r->owner_id].pending_current++;
                if (r->state == 2)
                    out->owners[r->owner_id].active_groups_current++;
            }
        }
    }
}
...
int vemb_v16_aeron_batch_poll_response(
    ...
    const uint8_t *nc_frame = ch->response_arena + (desc.start % ch->max_batch_bytes);
